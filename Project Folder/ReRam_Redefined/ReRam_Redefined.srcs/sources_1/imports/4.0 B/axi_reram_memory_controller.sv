...
module axi_reram_memory_controller #(
    ...
    logic [AXI_DATA_WIDTH-1:0] read_data_reg;  // Register to hold read data
    ...
    logic                         read_data_valid;   // Indicates read data is valid
...
//    int i, j;      // For loop variables (Vivado 2018.1 compatibility)
    ...
    always_comb begin
        ...
        if (write_state == AXI_WRITE_MEM && write_addr_valid) begin
            mem_addr = calculated_write_addr;
        end else if (read_state == AXI_READ_MEM && read_addr_valid) begin
    ...
    always_comb begin
        ...
        case (read_state)
            ...
            AXI_READ_ADDR: begin
                if (read_addr_is_memory) begin
                    if (read_addr_valid && addr_aligned) begin
                        read_state_next = AXI_READ_MEM;  // Valid memory read
                    end else begin
                        read_state_next = AXI_READ_DATA; // Invalid address/alignment
                    end
                end else begin
                    read_state_next = AXI_READ_DATA; // Non-memory (control) read
                end
            end
            ...
            AXI_READ_MEM: begin
                ...
                if (mem_done) begin
                    ...
                    read_state_next = AXI_READ_DATA;  // Go to data on error
                end
            end
            ...
            default: read_state_next = AXI_READ_IDLE;
        endcase
    end
    ...
    always_comb begin
        // Ready to accept new address when IDLE
        s_axi_arready = (read_state == AXI_READ_IDLE);
        
        // Valid response logic
        if (read_state == AXI_READ_DATA) begin
            if (read_addr_is_memory) begin
                s_axi_rvalid = read_data_valid && !mem_busy;  // Assert valid when data is ready and memory not busy
            end else if (read_addr_is_ctrl) begin
                s_axi_rvalid = 1'b1;  // Control register reads are always valid in data phase
            end else begin
                s_axi_rvalid = 1'b1;  // Error responses are always valid
            end
        end else begin
            s_axi_rvalid = 1'b0;
        end
        ...
        if (read_state == AXI_READ_DATA) begin
            if (!read_addr_is_memory && !read_addr_is_ctrl) begin
                ...
                if (read_data_valid) begin
                    ...
                    s_axi_rdata = read_data_reg;    // Use validated read data
                end else begin
                    s_axi_rresp = AXI_RESP_SLVERR;  // Data not ready
                    s_axi_rdata = 32'h0;
                end
    ...
    always_comb begin
        ...
        if (read_state == AXI_READ_MEM) begin
            if (!mem_busy && !read_in_progress) begin
                mem_read = 1'b1;  // Start read operation
            end
        end
        ...
        if (write_state == AXI_WRITE_MEM) begin
            if (!mem_busy) begin
                mem_write = 1'b1;  // Start write operation
                mem_data_in = write_data_reg;
            end
        end
    end
    ...
    always_ff @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            // Capture read data when memory operation completes
            if (read_state == AXI_READ_MEM) begin
                if (!read_in_progress && !mem_busy) begin
                    read_in_progress <= 1'b1;
                end else if (mem_done && read_in_progress) begin
                    read_data_reg <= mem_data_out;
                    read_data_valid <= 1'b1;
                    read_in_progress <= 1'b0;
                end
            end else if (read_state == AXI_READ_IDLE) begin
                read_data_valid <= 1'b0;
                read_in_progress <= 1'b0;
            end
