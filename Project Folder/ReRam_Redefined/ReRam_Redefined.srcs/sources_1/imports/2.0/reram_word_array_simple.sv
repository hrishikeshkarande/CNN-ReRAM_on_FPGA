...
always_ff @(posedge clk or negedge rst_n) begin
      if (!rst_n) begin
        ...
        // Default NOP unless we *just* left IDLE
        cell_command_reg <= CMD_NOP;
    
        case (current_word_state)
          WORD_IDLE: begin
            if (next_word_state != WORD_IDLE) begin
              // Emit a ONE-CYCLE command pulse on state entry
              unique case (next_word_state)
                WORD_READING: cell_command_reg <= CMD_READ;
                WORD_WRITING: cell_command_reg <= CMD_WRITE;
                default:      cell_command_reg <= CMD_NOP;
              endcase
              // Enable all cells for this operation
          ...
          WORD_READING: begin
            // Hold enables high while we wait; DO NOT keep asserting CMD_READ
            if (all_read_complete) begin
              cell_enables_reg <= {WORD_WIDTH{1'b0}};
            end else begin
              cell_enables_reg <= {WORD_WIDTH{1'b1}};
          ...
          WORD_WRITING: begin
            // Hold enables high while we wait; DO NOT keep asserting CMD_WRITE
            if (all_write_complete) begin
              cell_enables_reg <= {WORD_WIDTH{1'b0}};
            end else begin
              cell_enables_reg <= {WORD_WIDTH{1'b1}};
