...
module tb_reram_word_array_simple;
    ...
    parameter int MAX_ENDURANCE_CYCLES = 200;  // Single source of truth for endurance limit
    ...
    // Task helper variables (module-scope for Vivado 2018.1)
    int timeout_counter;
    ...
    reram_word_array_simple #(
        ...
        .MAX_WRITE_CYCLES   (MAX_ENDURANCE_CYCLES)  // Use single parameter
    ) dut (
        .clk                (clk),
        .rst_n              (rst_n),
        .command            (command),
    ...
    task automatic write_word(input [WORD_WIDTH-1:0] data);
        begin
            $display("[%0t] Writing word: 0x%08X (TB count: %0d, DUT cycles: %0d)", 
                     $time, data, total_writes_performed, total_write_cycles);
            @(posedge clk);
            write_data  = data;
            command     = CMD_WRITE;
            word_enable = 1;
            ...
            // Wait for completion with timeout in case cells are stuck
            timeout_counter = 0;
            while (!operation_complete && !word_error && timeout_counter < 100) begin
                @(posedge clk);
                timeout_counter++;
            end

            if (timeout_counter >= 100) begin
                $display("[ERROR] Write operation timeout - cells may be permanently stuck");
                $display("  operation_complete=%b, word_error=%b, cell_errors=0x%08X", 
                         operation_complete, word_error, cell_error_flags);
                $display("  TB count remains: %0d, DUT cycles: %0d", total_writes_performed, total_write_cycles);
                error_count++;
            end else if (word_error) begin
                $display("[ERROR] Write failed, cell_errors=0x%08X", cell_error_flags);
                $display("  This indicates cells are worn out and cannot be written");
                $display("  TB count remains: %0d, DUT cycles: %0d", total_writes_performed, total_write_cycles);
                if (cell_error_flags != 0) begin
                    $display("[CRITICAL] Cells may be worn out - no further writes should succeed");
                end
                error_count++;
            end else begin
                // Only increment TB counter if write was successful (matches DUT behavior)
                total_writes_performed++;
                $display("[%0t] Write completed successfully (TB count now: %0d, DUT cycles: %0d)", 
                         $time, total_writes_performed, total_write_cycles);
            end
        end
    endtask
    ...
    task automatic read_word(output [WORD_WIDTH-1:0] data);
        begin
            ...
            if (timeout_counter >= 100) begin
                $display("[ERROR] Read operation timeout - cells may be permanently stuck");
                $display("  operation_complete=%b, word_error=%b, cell_errors=0x%08X", 
                         operation_complete, word_error, cell_error_flags);
                error_count++;
                data = '0;
                ...
                $display("  This indicates cells are worn out and cannot be read");
                error_count++;
                data = '0;
            end else begin
                data = read_data;
    ...
    task automatic check_cell_health();
        begin
            ...
            $display("[HEALTH] Write utilization: %0d/%0d (%0d%%)",
                     total_writes_performed, MAX_ENDURANCE_CYCLES,
                     (total_writes_performed * 100) / MAX_ENDURANCE_CYCLES);
        end
    endtask
    ...
    initial begin
        ...
        // ------------------------- Phase 6 -------------------------
        test_phase = 6;
        $display("\n--- Phase 6: Endurance Counter Tracking ---");
        ...
        $display("Remaining writes to limit: %0d", MAX_ENDURANCE_CYCLES - total_writes_performed);

        endurance_writes = 5;  // Just a few test writes
        $display("Performing %0d endurance counter test writes...", endurance_writes);
        for (int e = 0; e < endurance_writes; e++) begin
            write_word(32'h12345678 + e);
            if (word_error) begin
                $display("Write error detected - stopping Phase 6 early");
                break;
            end
        end
        ...
        if (writes_after < writes_before) begin
            ...
            $display("[PASS] DUT write cycle counter advanced as expected");
        end
        ...
        for (int r = 0; r < 10; r++) begin
            ...
            if (word_error) begin
                $display("Word error detected - stopping random tests early");
                break;
            end
        end
        ...
        // ------------------------- Phase 9 -------------------------
        // Endurance Limit Testing - Hit the actual max cycles
        test_phase = 9;
        $display("\n--- Phase 9: Endurance Limit Testing ---");

        // Snapshot write counter
        writes_before = total_write_cycles;
        
        $display("Current state: TB writes=%0d, DUT write_cycles=%0d", 
                 total_writes_performed, total_write_cycles);
        $display("DUT configured with MAX_WRITE_CYCLES=%0d", MAX_ENDURANCE_CYCLES);
        
        // Use DUT counter as authoritative source for endurance limit
        remaining_to_limit = MAX_ENDURANCE_CYCLES - total_write_cycles;
        beyond_limit_writes = remaining_to_limit;  // Go a few beyond to verify permanent failure
        
        $display("DUT write cycles remaining to endurance limit: %0d", remaining_to_limit);
        $display("Planning %0d total writes to exceed limit and test permanent failure", beyond_limit_writes);
        
        // Reset error counters
        endurance_errors_detected = 0;
        writes_completed = 0;
        
        // Write until we hit and exceed the endurance limit
        for (int n = 0; n < beyond_limit_writes; n++) begin
            temp_pattern = (n % 2 == 0) ? 32'hDEADBEEF : 32'hCAFEBABE;
            
            $display("Phase 9 write %0d/%0d: pattern=0x%08X (TB_writes=%0d, DUT_cycles=%0d)", 
                     n+1, beyond_limit_writes, temp_pattern, total_writes_performed, total_write_cycles);
            
            write_word(temp_pattern);
            writes_completed = n + 1;

            // Monitor endurance status after each write
            if (word_error) begin
                $display("[ENDURANCE] Word error detected! (TB_writes=%0d, DUT_cycles=%0d)", 
                         total_writes_performed, total_write_cycles);
                endurance_errors_detected++;
            end
            
            if (cell_error_flags != '0) begin
                $display("[ENDURANCE] Cell error flags: 0x%08X (TB_writes=%0d, DUT_cycles=%0d)", 
                         cell_error_flags, total_writes_performed, total_write_cycles);
                ...
                $display("  %0d out of %0d cells showing endurance errors", error_cell_count, WORD_WIDTH);
                endurance_errors_detected++;
            end
            
            // Test read functionality after each write
            $display("  Testing read after write...");
            read_word(readback_data);
            
            if (word_error) begin
                $display("  [READ ERROR] Read also failing due to endurance issues");
                endurance_errors_detected++;
            end else if (readback_data !== temp_pattern) begin
                $display("  [DATA ERROR] Read data corruption detected!");
                ...
                $display("  [OK] Read verification successful");
            end
            
            // Check if we've hit the endurance limit (use DUT counter as authoritative)
            if (total_write_cycles >= MAX_ENDURANCE_CYCLES) begin
                $display("\n[ENDURANCE LIMIT HIT] DUT write_cycles reached MAX_ENDURANCE_CYCLES=%0d", MAX_ENDURANCE_CYCLES);
                $display("  TB writes performed: %0d", total_writes_performed);
                $display("  DUT write cycles: %0d", total_write_cycles);
                
                // Verify counters are synchronized (they should be equal for successful writes)
                if (total_writes_performed != total_write_cycles) begin
                    $display("  [WARNING] Counter mismatch: TB=%0d, DUT=%0d", total_writes_performed, total_write_cycles);
                    $display("  This suggests some writes failed and weren't counted by DUT");
                end
                
                // From this point on, cells should be permanently worn out
                if (endurance_errors_detected > 0) begin
                    $display("  [SUCCESS] Errors detected as expected when endurance limit exceeded");
                    
                    // Test that cells are now permanently unusable
                    $display("\n=== Testing Permanent Cell Failure ===");
                    for (int perm_test = 0; perm_test < 3; perm_test++) begin
                        temp_pattern = 32'h87654321 + perm_test;
                        $display("Permanent failure test %0d: attempting write 0x%08X", perm_test+1, temp_pattern);
                        
                        write_word(temp_pattern);
                        if (!word_error) begin
                            $display("  [UNEXPECTED] Write succeeded when cells should be permanently worn");
                        end else begin
                            $display("  [EXPECTED] Write failed - cells permanently worn");
                        end
                        
                        read_word(readback_data);
                        if (!word_error) begin
                            $display("  [INFO] Read still works, data: 0x%08X", readback_data);
                        end else begin
                            $display("  [EXPECTED] Read also failed - cells permanently worn");
                        end
                        
                        if (cell_error_flags == '1) begin // All bits high
                            $display("  [SUCCESS] All cell error flags permanently high: 0x%08X", cell_error_flags);
                        end else begin
                            $display("  [INFO] Cell error flags: 0x%08X", cell_error_flags);
                        end
                    end
                    break;  // Stop testing once we've verified permanent failure
                    
                end else begin
                    $display("  [UNEXPECTED] No errors detected even after exceeding endurance limit!");
                end
            end
            
            // Safety stop 
            if (writes_completed >= beyond_limit_writes) begin
                $display("[SAFETY] Completed planned endurance test writes");
                break;
            end
        end
        ...
        $display("  Final word error state: %b", word_error);

        // Final analysis
        if (total_write_cycles >= MAX_ENDURANCE_CYCLES && endurance_errors_detected > 0) begin
            $display("[SUCCESS] Phase 9 successfully tested endurance limits!");
            $display("  DUT write_cycles reached %0d limit, cells properly became unusable", MAX_ENDURANCE_CYCLES);
            $display("  Error flags and word_error properly indicate permanent failure");
            
            // Verify counter synchronization
            if (total_writes_performed == total_write_cycles) begin
                $display("  [GOOD] TB and DUT counters are synchronized");
            end else begin
                $display("  [INFO] Counter difference: TB=%0d, DUT=%0d", total_writes_performed, total_write_cycles);
                $display("         This is expected if some writes failed due to endurance");
            end
        end else if (endurance_errors_detected > 0) begin
            $display("[PARTIAL SUCCESS] Detected endurance errors before reaching limit");
            $display("  DUT cycles: %0d/%0d", total_write_cycles, MAX_ENDURANCE_CYCLES);
            $display("  This may indicate conservative endurance implementation");
        end else begin
            $display("[FAILURE] No endurance errors detected - endurance mechanism may not be working");
            $display("  DUT cycles reached: %0d/%0d", total_write_cycles, MAX_ENDURANCE_CYCLES);
        end
        ...
        $display("Testbench writes performed: %0d", total_writes_performed);
        $display("DUT write cycles counted: %0d", total_write_cycles);
        if (total_writes_performed != total_write_cycles) begin
            $display("Counter difference: %0d (failed writes not counted by DUT)", 
                     total_writes_performed - total_write_cycles);
        end
        $display("DUT endurance limit: %0d cycles", MAX_ENDURANCE_CYCLES);
        $display("Endurance margin remaining: %0d writes", MAX_ENDURANCE_CYCLES - total_write_cycles);
        ...
        // Calculate wear percentage based on DUT counter (authoritative)
        if (total_write_cycles > 0) begin
            wear_percentage = (total_write_cycles * 100) / MAX_ENDURANCE_CYCLES;
