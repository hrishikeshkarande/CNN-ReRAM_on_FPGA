...
module reram_memory_controller_blocking #(
    parameter int MEMORY_DEPTH = 8,        // Number of words (default: 16 words = 64B) - Conservative for Z7-20
    parameter int DATA_WIDTH = 16,           // 32-bit data
    parameter int ADDR_WIDTH = $clog2(MEMORY_DEPTH), // Auto-calculated from memory depth
    parameter int READ_DELAY_CYCLES = 6,     
    parameter int SET_DELAY_CYCLES = 20,     
    parameter int RESET_DELAY_CYCLES = 20,   
 ...
 //   output logic [7:0]             word_write_cycles [MEMORY_DEPTH-1:0], // Write cycles per word
  ...
  output logic [MEMORY_DEPTH-1:0]    word_cell_errors_any,  // Any cell errors per word
  // Flat outputs for Vivado 2018.1 compatibility
  output logic [MEMORY_DEPTH*8-1:0]           word_write_cycles_flat,
  output logic [MEMORY_DEPTH*DATA_WIDTH-1:0]  word_cell_errors_flat
);
    ...
    //==========================================================================
    // Output Flattening (Vivado 2018.1 Compatibility)
    //==========================================================================
    
    // Pack the internal 2D arrays into flat 1D output vectors
    genvar k;
    generate
        for (k = 0; k < MEMORY_DEPTH; k = k + 1) begin : flatten_outputs
            // Pack 8-bit counters into the flat vector
            assign word_write_cycles_flat[k*8 +: 8] = word_total_write_cycles[k];
    
            // Pack 32-bit error flags into the flat vector
            assign word_cell_errors_flat[k*DATA_WIDTH +: DATA_WIDTH] = word_cell_error_flags[k];
        end
    endgenerate
