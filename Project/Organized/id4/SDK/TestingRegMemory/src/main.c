...
#include "xil_printf.h"   // xil_printf (if using UART)
#include "xtime_l.h"

#define REG_BASE XPAR_REGISTERMEMORY_0_S00_AXI_BASEADDR  // from xparameters.h

/*
int main() {
    // Write a value to your 8-bit register
    Xil_Out32(REG_BASE, 0xA5);  // 0xA5 = 10100101

    // Read it back
    u32 value = Xil_In32(REG_BASE);

    // For debugging: if using UART/printf
...
int main() {
    ...
    while (1) {
        ...
        Xil_Out32(REG_BASE, 1 & 0xFF);  // write only lowest 8 bits
...
//        while (Xil_In32(REG_STATUS) & 0x1);
        ...
        // Read back result
        u32 val = Xil_In32(REG_BASE) & 0xFF; // read only lowest 8 bits
        xil_printf("Read value at addr 0: %d\n\r", val+1);

        xil_printf("Read @00 = 0x%X\r\n", Xil_In32(0x43C00000));
        xil_printf("Read @10 = 0x%X\r\n", Xil_In32(0x43C00010));
        xil_printf("Read @20 = 0x%X\r\n", Xil_In32(0x43C00020));
