 ...
 *     [31:16]: Reserved
 *   0x0004_0040 - 0x0004_007F: Per-Word Write Cycles (RO) - 16 words Ã— 32-bit
 ...
 *   0x0004_0080 - 0x0004_00BF: Per-Word Cell Error Details (RO) - 16 words Ã— 32-bit
...
module axi_reram_memory_controller #(
    ...
    always_comb begin
        ...
        if (read_state == AXI_READ_DATA) begin
            if (!read_addr_is_memory && !read_addr_is_ctrl) begin
                ...
                s_axi_rdata = read_data_reg;    // Use registered data
    ...
    always_comb begin
        ...
        if (!mem_op_active && !mem_busy && !mem_done) begin
            // Start new memory operations only when no operation is active
            if (read_state == AXI_READ_MEM && !read_in_progress) begin
                mem_read = 1'b1;  // Start read operation
            end else if (write_state == AXI_WRITE_MEM && write_addr_valid) begin
    ...
    always_ff @(posedge aclk or negedge aresetn) begin
        if (!aresetn) begin
            ...
            // Memory operation control and data capture
            if (!mem_op_active) begin
                if ((write_state == AXI_WRITE_MEM && write_addr_valid) || 
                    (read_state == AXI_READ_MEM && !read_in_progress)) begin
                    if (mem_read || mem_write) begin
                        mem_op_active <= 1'b1;  // Operation started
                    end
                end
            end else if (mem_done || mem_error) begin
                mem_op_active <= 1'b0;  // Operation complete
            end
            
            // Read data capture
            if (read_state == AXI_READ_MEM) begin
                if (!read_in_progress && mem_read) begin
            ...
            end else if (read_state == AXI_READ_IDLE || 
                        (read_state == AXI_READ_DATA && s_axi_rready && s_axi_rvalid)) begin
                read_data_valid <= 1'b0;
            end
