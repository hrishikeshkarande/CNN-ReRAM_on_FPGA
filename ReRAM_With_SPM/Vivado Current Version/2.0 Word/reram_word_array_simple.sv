...
module reram_word_array_simple #(
    ...
    always_comb begin
        ...
        // Check completion of operations - all cells either succeed OR error
        // This ensures operations complete even when cells are worn out
        all_read_complete = &(cell_read_success | cell_error_flag);   // All cells completed (success or error)
        all_write_complete = &(cell_write_success | cell_error_flag); // All cells completed (success or error)
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Command and enable logic based on current state
            case (current_word_state)
                WORD_IDLE: begin
                    if (next_word_state != WORD_IDLE) begin
                        // State transition from IDLE - start command
                        ...
                        endcase
                        // Enable all cells when starting operation
                ...
                WORD_READING: begin
                    // Maintain READ command until operation complete
                    if (all_read_complete) begin
                        cell_command_reg <= CMD_NOP;
                        cell_enables_reg <= {WORD_WIDTH{1'b0}};
                    end else begin
                        cell_command_reg <= CMD_READ;
                        cell_enables_reg <= {WORD_WIDTH{1'b1}};
                    end
                end
                
                WORD_WRITING: begin
                    // Maintain WRITE command until operation complete
                    if (all_write_complete) begin
                        cell_command_reg <= CMD_NOP;
                        cell_enables_reg <= {WORD_WIDTH{1'b0}};
                    end else begin
                        cell_command_reg <= CMD_WRITE;
                        cell_enables_reg <= {WORD_WIDTH{1'b1}};
                    end
                end
