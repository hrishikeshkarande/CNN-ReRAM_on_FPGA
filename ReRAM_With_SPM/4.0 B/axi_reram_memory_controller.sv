...
module axi_reram_memory_controller #(
    ...
    logic [31:0] word_cycles_packed [3:0];     // 4 registers for 16 words (4 bytes per reg)
    ...
    always_comb begin
        ...
        if (read_state == AXI_READ_DATA) begin
            if (!read_addr_is_memory && !read_addr_is_ctrl) begin
                ...
                s_axi_rdata = 32'hBAD0ADDR;     // Error pattern
            end else if (!addr_aligned) begin
                ...
                s_axi_rdata = 32'hERR0RERR;     // Error pattern
                ...
                logic [7:0] reg_addr = read_addr_reg[7:0];
                ...
                case (reg_addr[7:2])  // Use upper 6 bits for word addressing
                    ...
                    default: begin
                        if (reg_addr >= 8'h80 && reg_addr <= 8'hBF) begin
                            ...
                            logic [5:0] word_index = (reg_addr - 8'h80) >> 2;
