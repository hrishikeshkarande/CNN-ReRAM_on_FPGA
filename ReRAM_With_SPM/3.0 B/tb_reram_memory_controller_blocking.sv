...
module tb_reram_memory_controller_blocking;
    ...
    task read_memory(input [ADDR_WIDTH-1:0] address, output [DATA_WIDTH-1:0] data);
        begin
    ...
    task write_read_verify(input [ADDR_WIDTH-1:0] address, input [DATA_WIDTH-1:0] expected_data);
    ...
    task wear_out_address(input [ADDR_WIDTH-1:0] address, input [7:0] target_cycles);
        ...
        begin
            ...
            for (write_count = 0; write_count < remaining_cycles; write_count++) begin
                logic [DATA_WIDTH-1:0] test_data = 32'hAEAD0000 | write_count;
    ...
    task check_endurance_increment(input [ADDR_WIDTH-1:0] address, 
    ...
    initial begin
        ...
        for (int i = 0; i < 4; i++) begin
            logic [DATA_WIDTH-1:0] walking_1s = 1 << (i * 8); // Every 8th bit
            write_read_verify(4'h3 + i, walking_1s);
        end
        ...
        for (int i = 0; i < 8; i++) begin
            logic [ADDR_WIDTH-1:0] addr = i;
            logic [DATA_WIDTH-1:0] data = 32'h80000000 | i;
        ...
        for (int i = 0; i < 8; i++) begin
            logic [ADDR_WIDTH-1:0] addr = i;
            logic [DATA_WIDTH-1:0] expected = 32'h80000000 | i;
        ...
        logic [ADDR_WIDTH-1:0] test_addr1 = 4'h0;
        logic [ADDR_WIDTH-1:0] test_addr2 = 4'h1;
        logic [7:0] initial_cycles1, initial_cycles2;
        ...
        for (int i = 0; i < 10; i++) begin
            write_memory(test_addr1, 32'h12345678 + i);
        ...
        for (int i = 0; i < 5; i++) begin
            write_memory(test_addr2, 32'hABCDEF00 + i);
            $display("  Write %0d: cycles now = %0d", i+1, word_write_cycles[test_addr2]);
        end
        
        // Verify write cycle increments
        logic [7:0] final_cycles1 = word_write_cycles[test_addr1];
        logic [7:0] final_cycles2 = word_write_cycles[test_addr2];
        ...
        logic [7:0] pre_read_cycles = word_write_cycles[test_addr1];
        logic [DATA_WIDTH-1:0] dummy_data;
        
        // Perform multiple reads
        for (int i = 0; i < 5; i++) begin
            read_memory(test_addr1, dummy_data);
        end
        
        logic [7:0] post_read_cycles = word_write_cycles[test_addr1];
        ...
        logic [ADDR_WIDTH-1:0] wear_test_addr = 4'hF;
        logic [7:0] max_cycles = 200; // MAX_WRITE_CYCLES parameter
        
        $display("Wearing out address 0x%04X to exactly %0d write cycles...", wear_test_addr, max_cycles);
        
        // Record initial state
        logic [7:0] initial_wear_cycles = word_write_cycles[wear_test_addr];
        logic initial_cell_errors = word_cell_errors_any[wear_test_addr];
        ...
        logic [7:0] final_wear_cycles = word_write_cycles[wear_test_addr];
        logic final_cell_errors = word_cell_errors_any[wear_test_addr];
        ...
        logic [DATA_WIDTH-1:0] test_data = 32'hDEADBEEF;
        $display("Attempting write to worn-out address 0x%04X...", wear_test_addr);
        write_memory(wear_test_addr, test_data);
        ...
        logic [DATA_WIDTH-1:0] read_result;
        $display("Attempting read from worn-out address 0x%04X...", wear_test_addr);
        read_memory(wear_test_addr, read_result);
        ...
        logic [ADDR_WIDTH-1:0] good_addr = 4'h0;
        write_read_verify(good_addr, 32'h12345678);
        ...
        // Test invalid address (should complete with error)
        $display("Testing out-of-bounds address...");
        @(posedge clk);
        addr_i = 5'h10; // Address 16 is invalid (memory only has 0-15)
        data_i = 32'hDEADBEEF;
        write_i = 1;
        ...
        wait(done_o);
        if (error_o) begin
            $display("[PASS] Out-of-bounds address correctly flagged as error");
        end else begin
            $display("[ERROR] Out-of-bounds address should have generated error");
            error_count++;
        end
        ...
        if (done_o && error_o) begin
            ...
            $display("[ERROR] Simultaneous read/write should have generated immediate error");
            error_count++;
        end
